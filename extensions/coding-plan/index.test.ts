/**
 * Extension tests for the coding-plan quota poll + pi-web status shelf
 * (bun test). pi-web has no footer (`setFooter` is a no-op over RPC), so the
 * quota segment has to reach its extension-status shelf through `setStatus`.
 * These tests drive the extension with a fake ctx and a stubbed quota
 * endpoint:
 *
...
import registerCodingPlan from "./index"
...
/** Let the fire-and-forget poll (`void pollQuota`) reach its setStatus call. */
