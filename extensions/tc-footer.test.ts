/**
 * Quota-parsing tests for the tc-footer extension (bun test).
 *
 * Both parsers face third-party responses that changed underneath other
 * tools before (the OpenCode Go endpoint reshaped its payload within an hour
 * of launch — cc-switch#6433), so parsing is deliberately defensive: every
 * window is read independently, unusable windows are skipped, and a payload
 * with no usable window yields undefined (the footer then keeps the previous
 * snapshot instead of rendering garbage).
 *
 * Guards:
 *   - OpenCode Go: gauge order, per-gauge labels/baselines, ISO reset parsing;
 *   - OpenCode Go: `status: "ok"` + numeric `percent` gating, the 0% placeholder
 *     countdown drop, and undefined for unusable payloads;
 *   - GLM: unit 3 → ⏳5h (mdLink), unit 6 → ⏳7d (thinkingHigh), undefined
 *     without matching units.
 */
import { expect, test } from "bun:test"
import { parseGlmQuotas, parseGoQuotas } from "./tc-footer"
